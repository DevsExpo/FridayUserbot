...
from userbot.plugins.sql_helper.channel_sticker_sql import (
    add_new_data_in_db,
    is_data_indb,
    remove_datas,
)
...
async def _m(event):
    ...
    if event.is_group:
        await event.edit("`No, LoL You Can't Set Channel Stickers In Groups`")
        return
    if event.is_private:
        await event.edit(
            "`No, LoL You Can't Set Channel Stickers In Private Chats`"
        )
        return
