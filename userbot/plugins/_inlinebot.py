...
if Var.TG_BOT_USER_NAME_BF_HER is not None and tgbot is not None:
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:  # pylint:disable=E0602
            ...
            reply_popp_up_alert = "Please make your own bot, and don't use mine!"
...
async def on_plug_in_callback_query_handler(event):
    ...
    async def rip(event):
        ...
        await tgbot.send_message(
            LOG_CHAT,
            f"Hello, [This](tg://user?id={him_id}) Selected Probhited Option, Therefore Blocked.",
        )
...
Programmed and Powered by [CɪᴘʜᴇʀX](https://t.me/Hackintush) \nCurrently Loaded Plugins: {len(CMD_LIST)}"""
    ...
    async def rip(event):
        ...
        await tgbot.send_message(
            LOG_CHAT,
            message=f"Hello, [New User](tg://user?id={him_id}). Wants To Talk with You.",
