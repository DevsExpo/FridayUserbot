    ...
    async def inline_handler(event):
        ...
        if event.query.user_id == bot.uid and query == "dontpm":
            replied_user = await event.client(
                GetFullUserRequest(event.query.user_id))
            firstname = replied_user.user.first_name
            result = builder.article(
                title="PM Test",
                text=
                f"Hello {firstname}, This is Friday Security Service Of My Master [{DEFAULTUSER}](tg://user?id={bot.uid}). Please Click A Valid Reason Why You Are Here.",
                buttons=[
                    [
