...
async def gspider(fridaybot):
    ...
    await friday.edit(f"Global Ban Is Coming ! Wait And Watch You")
    ...
    if user:
        if user.id == 798271566:
            return await friday.edit(
                f"**Didn't , Your Father Teach You ? You Can't Gban Dev**"
            )
        try:
...
async def gspider(fridaybot):
    ...
    if user:
        if user.id == 798271566:
            return await friday.edit("**You Can't Ungban a Dev !**")
        try:
