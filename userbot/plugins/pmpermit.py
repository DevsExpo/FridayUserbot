...
USER_BOT_NO_WARN = ("**Hello, This is Friday PM Protection Service ⚠️**\n\n"
                    f"`My Master {DEFAULTUSER} is Busy Right Now !` \n"
                    "**You May Choose A Reason You Have Came For** \n\n"
                    f"**{CUSTOM_MIDDLE_PMP}**")
...
if Var.PRIVATE_GROUP_ID is not None:
    ...
    async def do_pm_permit_action(sender_id, event):
        if sender_id not in PM_WARNS:
            PM_WARNS.update({sender_id: 0})
        if PM_WARNS[sender_id] == 3:
