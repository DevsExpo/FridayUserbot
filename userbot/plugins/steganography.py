...
@friday.on(friday_on_cmd(pattern=r"stegno ?(.*)"))
@friday.on(sudo_cmd(pattern=r"stegano ?(.*)", allow_sudo=True))
async def hmm(event):
    if not event.reply_to_msg_id:
        await event.reply("Reply to any Image.")
        return
    await event.edit("Hiding Text Inside Image...")
    ...
    crypto_steganography.hide(img, "./cipherxdev/stegano.png", text)
...
@friday.on(friday_on_cmd(pattern=r"unstegno"))
@friday.on(sudo_cmd(pattern=r"unstegano", allow_sudo=True))
async def hmm(event):
    if not event.reply_to_msg_id:
        await event.reply("Reply to any Image.")
        return
    await event.edit("Searching for Text Inside The Image...")
    sed = await event.get_reply_message()
    if isinstance(sed.media, MessageMediaPhoto):
        img = await borg.download_media(sed.media, sedpath)
    elif "image" in sed.media.document.mime_type.split("/"):
...
\n\n**Syntax : **`.stegno <text> <Reply To A image>`\
\n**Usage :** Hides Given Text Inside Image.\
\n\n**Syntax : **`.unstegno <Reply To The image>`\
