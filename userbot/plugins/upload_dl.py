...
async def download(target_file):
    """ For .dl command, download files to the CɪᴘʜᴇʀX bot's server. """
    friday = await edit_or_reply(target_file, "`Processing ...`")
    await friday.edit("Processing using CɪᴘʜᴇʀX bot server ( ◜‿◝ )♡")
...
async def uploadir(udir_event):
    ...
    if os.path.exists(input_str):
        await udir_event.edit("Downloading Using CɪᴘʜᴇʀX bot Server....")
        lst_of_files = []
...
async def upload(u_event):
    """ For .upload command, allows you to upload a file from the CɪᴘʜᴇʀX bot's server """
